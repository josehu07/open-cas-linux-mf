...
class Experiment(object):
    def __init__(self):
        ...
        self.res_dir = self.home_dir + 'results/mix/' + self.exp_name
    ...
    def before_each(self, config):
        ...
        if self.tools_config['iostat']:
            ...
            shcmd('iostat -mx 1 /dev/cas1-1 /dev/nvme1n1 /dev/nvme0n1' + ' > ' + self.tmp_dir + '/iostat.out &')

        # start blktrace
        stop_blktrace_on_bg()
        if self.tools_config['blktrace']:
            start_blktrace_on_bg('/dev/cas1-1', self.tmp_dir +'/blktrace.output', ['issue', 'complete'])
    ...
    def exp(self, config):
        print '              *********** start running ***********'
        if 'mf' in config['type']:
            shcmd("bash ./run_with_mf_mix.sh " + config['type'] + "  "  + str(config['write_ratio']) + " " + str(config['read_write_shared_space']) + " " + str(config['qd']) + " >  " + self.tmp_dir + "/running")
        if 'mf' not in config['type']:
            shcmd("bash ./run_with_pure_mix.sh " + config['type'] + "  "  + str(config['write_ratio']) + " " + str(config['read_write_shared_space']) + " " + str(config['qd']) + " >  " + self.tmp_dir + "/running")
